	...
	var host = "https://app.bizplanner.com.ng"
	 ...
	 function callProfile(e,v){	
	  ...
	  $.ajax({
            ...
            success:function(data){				
				...
				/*  if(data.city){
				 document.getElementById("u_city").innerHTML += "<option  value='"+data.city+"' selected >" + data.city + "</options>";
				 }*/
...
function callStates() {
	...
	$(".update_doc").attr("disabled", "disabled");	
      ...
      dataType: 'jsonp',
      timeout: 50000,
      data: {email:email,lname:lname,fname:fname,mname:mname,gender:gender,phone:phone,state:state,address:address,bvn:bvn},
      success:function(data){ 
...
function regMem(){
  ...
  var delay =5000; 
    ...
    $.ajax({
      ...
      dataType: 'jsonp',
      timeout: 5000,
      data: {email:email,pwd:pwd,lname:lname,fname:fname},
      success:function(data){ 
	    if(data.symbol < 1) {
...
function addNewPlan(){
			...
			var l = 1;
         Lobibox.window({
				  title: '    <span style="margin-left:10px;"> ADD PLAN</span>',
					/*url: 'booking.html',*/
...
$.ajax({
	...
	success:function(data){
		 ...
		 var table =  $('#default-datatable').DataTable({
			...
			columns: [
                    ...
                    { data: 'percentage',
						"sClass":"left", 
						"mRender":function(data, type, full){							
						   return "<div style='float:left'> " + full.percentage + "% </div>";
						},
						title: "COMPLETION", className: "alignright"
                    },    
					{ data: 'custid', title: "CUSTOMER ID" },
					{ data: 'amt',
						"sClass":"left", 
						"mRender":function(data, type, full){							
		...
		"columnDefs": [{ "targets": -1, "data": null, "defaultContent":"<button class='btn btn-inverse-dark cursor-pointer manage-post' style='width:125px;'>MANAGE PLAN</button>"}],
		"scrollX": true,		
		"paging": true,
		"ordering":true,
		"responsive": true,
...
$(document).ready(function(){
  ...
  var $_GET = $_GET(),
      ip = "",
	  checkURL = ['post.html','dashboard.html','company.html','profile.html','favourites.html','listing.html','subscription.html','wallet.html'];
      url1 = "index.html",
	  url2 = "login.html",
	  ep = $_GET['ep'];	
...
//======================================//
//== Added Today 30/03/2020 by NnaDid ==//
//=====================================//
/*$.ajax({
	url:"https://api.restscene.com/pin",
	type: "GET",
	dataType: 'jsonp',
	timeout: 50000,
	data: {email:localStorage.email},
	success:function(data){
		console.log(data);
		  if(data.success==1){  // PIN exists Already
			$(".changePIN").slideUp(1000).show();
			$(".createPIN").slideDown(1000).hide();
		  }else{
			$(".changePIN").slideDown(1000).hide();
		  } 
		}
	});

$(document).on("submit" ,"#changePINForm, #createPINForm",function(evt){
	evt.preventDefault();
	...
	$.ajax({
		...
		dataType: 'jsonp',
		timeout: 50000,
		data: formData+'&email='+localStorage.email, 
		success:function(data){
			console.log(data);
			  if(data.success==1){  // PIN exists Already
				$(".changePIN").slideUp(1000).show();
				$(".createPIN").slideDown(1000).hide();
				$("#pinResult").html('<div class="alert alert-info">'+data.msg+'</div>');
			  }else{
				$(".changePIN").slideDown(1000).hide();
				$("#pinResult").html('<div class="alert alert-info">'+data.msg+'</div>');
			  } 
